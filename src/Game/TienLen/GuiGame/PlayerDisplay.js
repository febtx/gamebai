//
TienLen.PlayerDisplay = cc.Node.extend({
    ctor: function() {
        this._super();
        this.handOnCards = [];
        this.baiEndGame = [];
    ...
    initWithCards: function(cards){
        ...
        for(i = 0; i < cards.length; i++){
            var cardSprite = new TienLen.CardSprite(cards[i]);
    ...
    firstTurn: function(id){
        ...
        if(this.index == 0){
            ...
            this.cardFirstTurn.setId(id);
            //this.cardFirstTurn.removeFromParent();
            //this.cardFirstTurn = new cc.Sprite(ss);
    ...
    stopEffectTime: function()
    {
        ...
        if(this.panel.getChildByName("bg_progress").getChildByTag(TienLen.PlayerDisplay.sunnodetag))
        {
            this.panel.getChildByName("bg_progress").getChildByTag(TienLen.PlayerDisplay.sunnodetag).stopAllActions();
            this.panel.getChildByName("bg_progress").getChildByTag(TienLen.PlayerDisplay.sunnodetag).removeFromParent(true);
        }
    },
    ...
    addEffectTime: function(time,timeRemain){
        ...
        if(!cc.sys.isNative){
            ...
            effect = new cc.ProgressFromTo(time, percent*100, 0);
        }
        ...
        node.setTag(TienLen.PlayerDisplay.sunnodetag);
        ...
        var action = !cc.sys.isNative?engine.MoveCircle.create(time, size.width/2 -2, 0, 6.28) : cc.MoveCircle.create(time,size.width/2 -2, 0, 6.28);
    ...
    initMyPlayer:function(){
        this.sortType = TienLen.kSortTangDan;
        this.setPosition(cc.p(251, 12));
    ...
    initPlayerDisplay: function(){
        ...
        var cardTemp = new TienLen.CardSprite(52);
        ...
        //avatar.setPosition(cc.p(ccui.Helper.seekWidgetByName(this.panel,"btnAvatar").getPosition().x + 200, ccui.Helper.seekWidgetByName(this.panel,"btnAvatar").getPosition().y + 200) );
        avatar.setPosition(this.panel.getChildByName("btnAvatar").getPosition());
        //avatar.setPosition(ccui.Helper.seekWidgetByName(this.panel,"btnAvatar").getPosition());
        ...
        if(this.index == 0){
            this.cardFirstTurn = new TienLen.CardSprite(52);
            ...
            this.cardFirstTurn = new TienLen.CardSprite(52);
            var cardSize = this.cardFirstTurn.getContentSize();
            var cardFirstTurnSize = this.cardFirstTurn.getContentSize();
            this.setScale(cardFirstTurnSize.width/cardSize.width);
        ...
        this.uiBaosam = GuiUtil.createSprite(TienLen.res.iconBaoSamPng);
        ...
        var cardSize = (GuiUtil.createSprite(TienLen.res.cardFirstTurnPng)).getContentSize();
        var xBaoSam = GuiUtil.createSprite(TienLen.res.xBaoSamPng);
        ...
        var sprite = GuiUtil.createSprite(TienLen.res.circleEffectPng);
    ...
    setAvatar: function(avatarType){
        //this.avatar.setTexture(cc.textureCache.addImage("res/common/avatar/Avatar_" + avatarType + ".png"));
    ...
    sapXep: function(){
        ...
        var cardsSapXep = [];
        var originalCards = [];
        if(this.sortType == TienLen.kSortTangDan){
            this.sortType = TienLen.kSortGroup;
        }else{
            this.sortType = TienLen.kSortTangDan;
        }
        ...
        for(i = 0; i < this.handOnCards.length; i++){
            ...
            cardsSapXep.push(new TienLen.Card(this.handOnCards[i].id));
            originalCards.push(new TienLen.Card(this.handOnCards[i].id));
        }

        var res = TienLen.GameHelper.sapxepQuanBai(cardsSapXep, this.sortType);
    ...
    boluot: function()
    {
        ...
        if(parent.getChildByTag(TienLen.PlayerDisplay.boluottag)) {
            parent.getChildByTag(TienLen.PlayerDisplay.boluottag).removeFromParent(true);
        }
        ...
        boluot.setScale(0);
        boluot.runAction(new cc.EaseBackOut(cc.scaleTo(0.15,1.1)));
        boluot.setTag(TienLen.PlayerDisplay.boluottag);
    ...
    clearBoluot: function()
    {
        ...
        if(parent.getChildByTag(TienLen.PlayerDisplay.boluottag))
        {
            parent.getChildByTag(TienLen.PlayerDisplay.boluottag).stopAllActions();
            parent.getChildByTag(TienLen.PlayerDisplay.boluottag).setScale(1.1);
            parent.getChildByTag(TienLen.PlayerDisplay.boluottag).runAction(cc.sequence(cc.delayTime(0.5),cc.fadeOut(0.1),cc.hide()));
        }
    },
    ...
    clearBaiEndGame: function()
    {
        ...
        //    this.baiEndGame[i].runAction(cc.sequence(cc.fadeOut(.1),cc.removeSelf()));
        //}
        //this.baiEndGame = [];
            ...
            case 4:
            {
                ...
                else
                {
                    ...
                    for(var i=0;i<5;i++){
                        var sam = new TienLen.CardSprite(cards[i]);
                        sam.setScale(.72);
                        this.addChild(sam, TienLen.MAX_CARDS - i);
                        this.baiEndGame.push(sam);
                        sam.setPosition(cc.p(centerPos.x - deltaX* i, startY - 40));
                        sam.setOpacity(0);
                        sam.runAction(cc.sequence(cc.delayTime(time),cc.fadeIn(.45)));
    ...
    addThang: function(delayTime, samthanhcong)            // Them effect thang'
    {
        ...
        ////var sprite1 = new cc.Sprite(TienLen.res.chienThangPng);
        //
        var time = .5;
        ...
        if(this.index == 0)
        {
            var sprite2 = GuiUtil.createSprite(TienLen.res.nenChienThang);
            var sprite1 = GuiUtil.createSprite(TienLen.res.chienThang);
    ...
    addThua: function(delayTime)
    {
        ...
        if(this.index == 0)
        {
            var sprite2 = GuiUtil.createSprite(TienLen.res.nenThuaCuoc);
            var sprite1 = GuiUtil.createSprite(TienLen.res.thuaCuoc);
            sprite2.addChild(sprite1);
            sprite1.setPosition(sprite2.getContentSize().width/2, sprite2.getContentSize().height/2);
    ...
    addHoa: function(delayTime)
    {
        ...
        {
            var sprite2 = GuiUtil.createSprite(TienLen.res.nenThuaCuoc);
            var sprite1 = GuiUtil.createSprite(TienLen.res.hoaPng);
            sprite2.addChild(sprite1);
            sprite1.setPosition(sprite2.getContentSize().width/2, sprite2.getContentSize().height/2);
    ...
    initBao1: function(){
        ...
        if ((this.index == 3) || (this.index == 4)) {
            var bao1 = GuiUtil.createSprite(TienLen.res.bao1bPng);
            ...
            var bao1 = GuiUtil.createSprite(TienLen.res.bao1aPng);
            this.card.addChild(bao1);
            this.bao1 = bao1;
            bao1.setLocalZOrder(1);
            this.card.getChildByName("num").setLocalZOrder(2);
    ...
    addMoney: function(money, time)           // Effect tien` khi het van' choi
    {
        ...
        this.updateMoney = function(sender){
            this.uiGold.setString(StringUtility.standartNumber(TienLen.gameLogic.players[this.index].info["money"]));
        }
...
TienLen.PlayerDisplay.sunnodetag = 111;
TienLen.PlayerDisplay.boluottag = 15;
